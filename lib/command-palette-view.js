...
export default class CommandPaletteView {
  constructor () {
    ...
    this.selectListView = new SelectListView({
      ...
      apmMessage: 'Run atom package manager',
      ...
      didConfirmApmCommand: (command) => {
        this.hide()
        const event = new CustomEvent('command-palette:apm', {
          bubbles: true,
          cancelable: true,
          detail: command
        })
  ...
  async show () {
    ...
    this.commandsForActiveElement = atom.commands.findCommands({target: this.activeElement}).filter(cmd => {
      return cmd.name !== 'command-palette:apm'
    })
